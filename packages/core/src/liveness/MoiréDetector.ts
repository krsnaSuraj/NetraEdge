/**
 * MoiréDetector — FFT-based screen spoof detection.
 *
...
export interface MoiréResult {
  readonly moiréScore: number;
  readonly peaks: readonly number[];
  readonly periodicity: number;
  readonly detected: boolean;
}

const MOIRÉ_THRESHOLD = 0.4;
const MIN_PEAK_MAGNITUDE = 2.5;
const FREQ_LOW = 8;
const FREQ_HIGH = 40;

export function detectMoiré(
  ...
  const moiréScore = Math.min(1, peaks.length * 0.15 + periodicity * 0.5);

  return { moiréScore, peaks, periodicity, detected: moiréScore > MOIRÉ_THRESHOLD };
}
